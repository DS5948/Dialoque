...
function addChatClickEvents() {
  ...
  for (let i = 0; i < chat.length; i++) {
    chat[i].addEventListener("click", async() => {
      if (selectedChat) {
      ...
      await getMessages(selectedChatId, loggedInEmail);
      await getLatestMessage(selectedChatId);
      if (chatMsgList.scrollHeight > chatMsgList.clientHeight) {
        chatMsgList.scrollTop = chatMsgList.scrollHeight - chatMsgList.clientHeight;
      }
    });
    ...
    if (loggedInEmail === friendEmail) {
      ...
      for (let i = 0; i < friendsList.length; i++) {
        if (friendsList[i].getAttribute("data-userId") === idToUpdateCount) {
          ...
          if(selectedChatId === friendsList[i].getAttribute("data-userId")) {
            console.log(selectedChatId,friendsList[i].getAttribute("data-userId"),msgCountSpan.style.display);
          }
          else {
